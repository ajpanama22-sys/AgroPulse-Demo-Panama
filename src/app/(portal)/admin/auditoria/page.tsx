            ...
            <tbody>
              {cumplimiento.map((c) => (
                <tr key={c.ubicacion.id} className="border-b border-border/60">
                  ...
                  <td className="py-2.5 pr-4 text-text-muted">{c.sincronizadoEn ? new Date(c.sincronizadoEn).toLocaleTimeString("es-VE") : "—"}</td>
            ...
            <tbody>
              {log.map((l) => (
                <tr key={l.id} className="border-b border-border/60">
                  <td className="py-2 pr-4 whitespace-nowrap text-text-muted">{new Date(l.creadoEn).toLocaleString("es-VE")}</td>
